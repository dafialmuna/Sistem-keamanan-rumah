...
// Wi-Fi credentials and Telegram bot are stored in secrets.h
#include "secrets.h"
...
// Memulai proses koneksi WiFi
  WiFi.begin(ssid, password);
  WiFi.setSleep(false); // Mencegah WiFi mati otomatis untuk menghemat daya
  // Melakukan perulangan (menunggu) selama WiFi belum terhubung
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print("."); // Menampilkan titik-titik di Serial Monitor saat loading
  }
  // Jika sudah keluar dari perulangan (berhasil terhubung)
  Serial.println("\nWiFi connected");
